...
function createGroup(name, lights, cb) {
  var endpoint = '/groups';
  var data = {
    name: name,
    lights: lights
...
function setGroupState(id, state, cb) {
  var endpoint = util.format('/groups/%s/state', encodeURIComponent(id));
  var data = state;
...
function modifyScene(id, light, state, cb) {
